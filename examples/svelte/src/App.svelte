<script lang="ts">
  ...
  import { CoreLightbox } from '@anil-labs/lightbox-gallery-svelte'
  ...
  let ssGrid = $state<HTMLElement>()
  let inlineStage = $state<HTMLElement>()
  let apiGrid = $state<HTMLElement>()
  let log = $state<{ k: string; d: string }[]>([])
  let ss: CoreLightbox
  let inlineLb: CoreLightbox
  let api: CoreLightbox

  const push = (k: string, d = ''): void => {
    log = [{ k, d }, ...log].slice(0, 40)
  }
  const playSlideshow = (): void => {
    ss.open(0)
    ss.toggleSlideshow()
  }
  ...
  onMount(() => {
    ss = new CoreLightbox({
      ...
      animateFrom: (i) => ssGrid?.querySelectorAll('img')[i] ?? null,
    })
    ...
    api = new CoreLightbox({
      ...
      slideshow: true,
      fullscreen: true,
      animateFrom: (i) => apiGrid?.querySelectorAll('img')[i] ?? null,
    })
...
<main>
  ...
  <section class="section" id="basic">
    <div class="section-head">
      ...
      <p class="desc">
        Click a thumbnail and it opens with a FLIP zoom. Wheel/pinch zoom, swipe, arrows, <kbd
          >f</kbd
        >
        and <kbd>Esc</kbd> all work.
      </p>
  ...
  <section class="section" id="media">
    <div class="section-head">
      ...
      <p class="use"><b>Use it for</b> mixed image + video galleries, case studies, press kits.</p>
    </div>
  ...
  <section class="section" id="slideshow">
    ...
    <div class="card-wrap">
      <div class="toolbar">
        <button class="btn primary" onclick={playSlideshow}>▶ Play slideshow</button>
      </div>
      <div class="thumbs" bind:this={ssGrid}>
        {#each PHOTOS as item, i (i)}
          <button class="thumb" onclick={() => ss.open(i)}>
  ...
  <section class="section" id="inline">
    <div class="section-head">
      ...
      <p class="desc">
        <code>inline: true</code> with a <code>container</code> renders a zoomable, swipeable carousel
        on the page.
      </p>
      <p class="use"><b>Use it for</b> product pages, embedded galleries, dashboards.</p>
    </div>
    <div class="card-wrap"><div class="inline-stage" bind:this={inlineStage}></div></div>
  </section>
  ...
  <section class="section" id="hash">
    <div class="section-head">
      ...
      <p class="desc">
        <code>hash: true</code> writes the open slide to the URL, so links are shareable and the back
        button closes the viewer.
      </p>
      <p class="use"><b>Use it for</b> shareable photos, SEO-friendly galleries, back-button UX.</p>
    </div>
  ...
  <section class="section" id="api">
    ...
    <div class="card-wrap">
      <div class="toolbar">
        <button class="btn" onclick={() => api.open(0)}>Open</button>
        <button class="btn" onclick={() => api.prev()}>‹ Prev</button>
        <button class="btn" onclick={() => api.next()}>Next ›</button>
        <button
          class="btn"
          onclick={() => {
            ensureApi()
      ...
      <div class="thumbs" bind:this={apiGrid}>
        {#each PHOTOS as item, i (i)}
          <button class="thumb" onclick={() => api.open(i)}>
            <img src={item.thumb} alt={item.caption} loading="lazy" />
          </button>
        {/each}
      </div>
